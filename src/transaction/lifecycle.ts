...
export type TransactionPhaseStatusError =
  "FAILED" | "EXCEPTION"

export type TransactionPhaseStatusNoError =
  "IN_PROGRESS" | "SUCCESS"

export type TransactionPhaseStatus =
  TransactionPhaseStatusError | TransactionPhaseStatusNoError
...
export class TransactionLifecycleApi implements ITransactionLifecycleApi {
  ...
  private updateStatusWithErrors(status: TransactionPhaseStatusError, errors: any[]) {
    const currentStatus = this.getStatus();
    const { phase } = currentStatus;

    const newErrors = (currentStatus.status === "FAILED" || currentStatus.status === "EXCEPTION")
