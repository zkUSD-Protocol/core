...
describe('zkUSD Multisig Council Test Suite', () => {
  ...
  let senderKeys = () => testHelper.agents.alice.keys;
  ...
  it('should be possible for a council member to create a proposal', async () => {
    ...
    const rootBefore = (await proposalMap()).getRoot()
    if (!rootBefore) {
      throw new Error('Proposal root is undefined');
    }
    const onchainRoot = await cclient().councilContract.proposalsMerkleMapRoot.fetch();
    if(!onchainRoot){
      throw new Error('Proposal root is undefined');
    }
    assert.ok(
      onchainRoot.equals(rootBefore).toBoolean(),
      'Proposal root does not match'
    );

    const result = await cclient().submitVote(voteProof, senderKeys() , {force: true});
    ...
    // proposal map should have changed
    assert.ok(newRoot.equals(rootBefore).not().toBoolean(), 'Proposal map did not change');
...
//     const result = await cclient().tryPassProposal(updateSpec, senderKeys());
//     assert.ok(!result.transactionIncluded, 'Transaction was included');

//     // now lets test the force flag, it should throw
//     assert.rejects(async () => {
//       await cclient().tryPassProposal(updateSpec, senderKeys(), { force: true });
//     });
...
//     const contractEventsBefore = await cclient().councilContract.fetchEvents();
//     const result = await cclient().submitVote(newProof, senderKeys());
...
//       seat: 1,
//     });
//     const submitResult = await cclient().submitVote(voteProof, senderKeys());
...
//     const contractEventsBefore = await cclient().councilContract.fetchEvents();
//     const passResults = await cclient().tryPassProposal(updateSpec, senderKeys());
...
//       anotherVoteProof
//     );
//     const result = await cclient().submitVote(mergedVotesProof, senderKeys());
//     assert.ok(result.transactionIncluded, 'Transaction was not included');
//     assert.ok(!result.votesMissing, 'Votes missing');
...
//     // saving events
//     const contractEventsBefore = await cclient().councilContract.fetchEvents();
//     const passResult = await cclient().tryPassProposal(updateSpec, senderKeys());
