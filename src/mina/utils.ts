...
/** Extracts all parties involved in a tx that may require a local state updated. */
function extractAllTxPartiesJson(zkAppCommand: SignerZkappCommand): Set<Account> {
  const parties = new Set<Account>();
  ...
  if (zkAppCommand.feePayer) {
    parties.add({ publicKey: PublicKey.fromBase58(zkAppCommand.feePayer.feePayer) });
  }
  else {
    parties.add({ publicKey: PublicKey.fromBase58(tx.feePayer.body.publicKey) });
  }
  for (let au of tx.accountUpdates) {
    parties.add({ publicKey: PublicKey.fromBase58(au.body.publicKey), tokenId: TokenId.fromBase58(au.body.tokenId) });
    const mdelegate = au.body.update.delegate;
    if (mdelegate) {
