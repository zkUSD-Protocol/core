 ...
 */
export function getNewCouncilMembers(events: any[]): PublicKey[] {
  ...
  const newCouncilEvent =
    eventData as unknown as NewCouncilInitializedWithFixedKeysEvent;
  ...
  const nonEmptyMembers = councilMemberArray.filter(
    (pk) => !pk.isEmpty().toBoolean()
  );
