 ...
 */
export class ResolutionTreeContractEventsProvider
  implements IMerkleDataProvider<ResolutionTree>
{
  ...
  constructor(
    ...
    private readonly fetchCurrentBlockHeight: FetchCurrentBlockHeight = async () =>
      undefined,
  ...
  static fromContract(
    ...
    chunkSize = 1000
  ): ResolutionTreeContractEventsProvider {
    return new ResolutionTreeContractEventsProvider(
      councilContract,
  ...
  public static applyEvents(
    ...
    const proposalPassedEvents = events.filter(isProposalPassedEvent);
    const sortedEvents = proposalPassedEvents.sort((a, b) =>
      a.blockHeight.toBigint() < b.blockHeight.toBigint() ? -1 : 1
    );
    sortedEvents.forEach((e) =>
      resolutionTree.setLeaf(
        e.event.data.resolutionIndex.toBigint(),
