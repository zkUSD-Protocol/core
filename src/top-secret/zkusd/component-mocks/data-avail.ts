...
import { StateRoots, NextStateCandidate, FullState, SystemParams, stateRootsEqual } from "../validator/block-state.js";
...
class DataAvailMock implements DataAvailInterface{
    ...
    private get _stateMap(): Map<string, State>{
        ...
        map.set(this._finalizedState.metadata.metadataBlobId, this._finalizedState);
        if(this._candidateState){
            map.set(this._candidateState.metadata.metadataBlobId, this._candidateState);
        }
        return map;
    }
    ...
    async syncViaMetadataBlob(args: { localStateProxy: LocalStateProxy; metadataBlobId: string; }): Promise<void> {
        if(!this._inited){
            throw new Error("Data availability not initialized");
        }
        // find statet in map by metadata blob id
        const state = this._stateMap.get(args.metadataBlobId);
        if(!state){
    ...
    async publishBlockUpdate(finalizedState: LocalStateProxy, nextBlockStateCandidate: NextStateCandidate): Promise<StateStoreMetadata> {
        ...
        if(!stateRootsEqual(currentStateRoots, candidateFinalizedStateRoots)){
            throw new Error("Current state and candidate finalized state are not equal");
        }
        ...
        candidateState.applyMapOperations(...nextBlockStateCandidate.intentOperations);
    ...
    private computeCandidateMetadata(hash:string): StateStoreMetadata{
        return {
            ...
            metadataBlobId: hash + 'candidateMetadataBlobId'
        }
