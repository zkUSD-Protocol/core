...
export class SqliteIntentProofStore implements IntentProofStore {
  ...
  storeProof(proof: IntentProof): string {
    const key = IntentProofHelper.hash(proof);
    const json = JSON.stringify(proof.proof.toJSON());

    this.db.prepare(this.insertStmt).run(key, proof.kind, json, Date.now());

    return key;
  }

  async getProof(hash: string): Promise<IntentProof | null> {
    const row = this.db.prepare(this.selectStmt).get(hash) as
      | { kind: IntentProofKind; proof_json: string }
      | undefined;
    ...
    switch (row.kind) {
      case 'burn':
        return { kind: 'burn', proof: await BurnIntentProof.fromJSON(parsed) };
      case 'mint':
        return { kind: 'mint', proof: await MintIntentProof.fromJSON(parsed) };
      case 'transfer':
        return {
          kind: 'transfer',
          proof: await TransferIntentProof.fromJSON(parsed),
        };
      case 'redeem':
        return {
          kind: 'redeem',
          proof: await RedeemIntentProof.fromJSON(parsed),
        };
      case 'create-vault':
        return {
          kind: 'create-vault',
          proof: await CreateVaultIntentProof.fromJSON(parsed),
        };
      case 'deposit':
        return {
          kind: 'deposit',
          proof: await DepositIntentProof.fromJSON(parsed),
        };
      case 'liquidate':
        return {
          kind: 'liquidate',
          proof: await LiquidateIntentProof.fromJSON(parsed),
        };
      /* c8 ignore next */
      default:
