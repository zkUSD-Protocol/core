...
export function Vault(params: VaultParams) {
  const VaultClass = class Vault_ extends Struct(VaultState) {
    ...
    static unpack(packedVault: Field): Vault_ {
      const bitFields = [
        { name: 'type', length: 8 },
    ...
    public calculateMinaValue(usdValue: Field, minaPrice: MinaPrice): Field {
      return this.fieldIntegerDiv(
        usdValue.mul(Vault_.UNIT_PRECISION),
    ...
    public calculateMaxAllowedDebt(collateralValue: Field): Field {
      const numCollateralValue = collateralValue.mul(
        Vault_.COLLATERAL_RATIO_PRECISION
      );

      const maxAllowedDebt = this.fieldIntegerDiv(
        numCollateralValue,
        Vault_.COLLATERAL_RATIO
      ).mul(Vault_.COLLATERAL_RATIO_PRECISION);
    ...
    public computeLiquidationAmounts(args: {
      ...
      const liquidatorMaxCollateral = this.fieldIntegerDiv(
        liquidatedDebtMina.mul(Vault_.LIQUIDATION_BONUS_RATIO),
        Field(100)
      );
