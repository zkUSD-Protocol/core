...
export interface ZkusdStateManager {
  ...
  finishIntentEpoch(): Promise<void>;
}
...
export class InMemoryZkusdStateManager implements ZkusdStateManager {
  ...
  async setState(s: ZkUsdState)  { this.state = s; }
  async finishIntentEpoch() {
...
export interface StateEventProcessor {
 ...
 applyEpochEnd(pochEnd: EpochEndSignal): Promise<OptimisticState>;
    ...
    while (this.running && (steps === undefined || i < steps)) {
      ...
      try {
        if (evt.kind === 'intent-proof') {
          await this.processor.verifyAndApplyIntent(evt, { requeueIfNoProof: true });
        } else {                                             // epoch-end
          console.log('Applying epoch end');
          const state = await this.processor.applyEpochEnd(evt);
          // clone maps 
          this._intentVaultMap = cloneVaultMap(await this.processor.liveVaultMap());
          this._intentZkUsdMap = cloneZkUsdMap(await this.processor.liveZkUsdMap());
...
export class OptimisticStateProcessor implements StateEventProcessor {
  ...
  >(proof: P, rollupFn: Fn, needsZkUsd = false) => {
    ...
    try {
      const { publicOutput } = needsZkUsd
        ? await rollupFn(prev, proof.proof, this._liveVaultMap, this._liveZkUsdMap)
  ...
  private readonly handlers: Record<IntentProofKind, ProofHandler<any>> = {
    'transfer'    : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.transfer, true),
    'mint'        : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.mintZkUsd, true),
    'burn'        : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.burnZkUsd, true),
    'redeem'      : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.redeemCollateral),
    'liquidate'   : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.liquidate, true),
    'deposit'     : p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.depositCollateral),
    'create-vault': p => this.runProof(p, Rollup.ZkUsdRollup.rawMethods.createVault),
  };
  ...
  async verifyAndApplyIntent(
    ...
    if (commitment.intentStateRoots.vaultMapRoot) {
      // check if valid
      const validVaultMapRoot = commitment.intentStateRoots.vaultMapRoot.equals(roots.intentVaultMapRoot).toBoolean();
      if (!validVaultMapRoot) {
        this.log.push(`Invalid vault map root in commitment ${commitment.commitmentId}`);
        return { success: false, reason: 'invalid vault map root' };
      }
    }
    if (commitment.intentStateRoots.zkUsdMapRoot) {
      // check if valid
      const validZkUsdMapRoot = commitment.intentStateRoots.zkUsdMapRoot.equals(roots.intentZkUsdMapRoot).toBoolean();
      if (!validZkUsdMapRoot) {
        this.log.push(`Invalid zkUSD map root in commitment ${commitment.commitmentId}`);
  ...
  async processPendingMissing() {
    ...
    for (const c of items) await this.verifyAndApplyIntent(c, { requeueIfNoProof: false });
  }

  async applyEpochEnd(_: EpochEndSignal): Promise<OptimisticState> {
    console.log('Applying epoch end');
    await this.localState.finishIntentEpoch();
    return { intentEpochState: await this.localState.getState() };
  }

  async getStateProcessorLog() { return this.log; }
}