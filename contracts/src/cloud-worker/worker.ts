...
export class zkUsdWorker extends zkCloudWorker {
  ...
  private async compile(compileSmartContracts: boolean = true): Promise<void> {
    try {
      ...
      const networkKeys = getNetworkKeys(this.cloud.chain);

      if (zkUsdWorker.contractVerificationKey === undefined) {
        console.time('compiled AddContract');
        zkUsdWorker.contractVerificationKey = (
  ...
  public async create(transaction: string): Promise<string | undefined> {
    ...
    if (zkUsdWorker.programVerificationKey === undefined)
  ...
  public async merge(
    ...
    const ok = await verify(
      output.proof.toJSON(),
      zkUsdWorker.programVerificationKey
    );
  ...
  private async verifyProof(args: { proof: string }): Promise<string> {
    ...
    const ok = await verify(proof, zkUsdWorker.programVerificationKey);
